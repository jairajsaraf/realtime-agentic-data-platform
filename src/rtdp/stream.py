...
live sources with rate limits). Transient per-batch errors are caught with exponential backoff so
the loop survives them; the loop stops cleanly on ``KeyboardInterrupt``.
"""
    ...
    *,
    ...
    Args:
        ...
        on_batch / on_skip / on_error: optional callbacks for a successful append, a skipped
            empty batch, and a caught per-batch error, respectively.
